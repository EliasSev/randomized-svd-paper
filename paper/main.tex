...
Consider the goal of finding a rank-$k$ approximation to an $m \times n$ matrix $A$. We seek a matrix $X$ with $\operatorname{rank}(X) \le k$ which approximates $A$. The problem formulation is
\begin{equation}\label{probForm1}
    \min_{\operatorname{rank}(X)\le k} \|A - X \|_2,
\end{equation}
...
Let $Q\in\mathbb R^{m\times k}$ have orthonormal columns, i.e., $Q^TQ = I_k$. Then $QQ^TA$ is the orthogonal projection of $A$ onto the $k$-dimensional subspace $\operatorname{span}(Q)$. The problem can also be formulated as
...
In 1936, C. Eckart and G. Young showed that the solution to Equation~\eqref{probForm1} is $X = U_k\Sigma_k V_k^T$, where $U_k$ and $V_k$ consist of the first $k$ left and right singular vectors of $A$, respectively, and $\Sigma_k$ contains the first $k$ singular values of $A$ \cite{eckart_approximation_1936}. This is known as the Eckart–Young–Mirsky theorem. Alternatively, the solution $Q$ of Equation~\eqref{probForm2} consists of the first $k$ left singular vectors of $A$. This implies that
...
Noting that $\sigma_j \le \sigma_{k+1}$ for all $j > k$, Equation~\eqref{eq:rsvd_error} implies that
...
Python 3.13 was used for all computations, together with NumPy and SciPy for linear algebra routines \cite{2020NumPy-Array,2020SciPy-NMeth}. The implementation of Algorithm~\ref{alg:rsvd} was done using the QR and SVD routines provided by SciPy, which were specified to use the GESDD solver for SVD computations. All library specifications and code used to generate the results in this paper are openly available at: \href{https://github.com/EliasSev/randomized-svd-paper}{github.com/EliasSev/randomized-svd-paper}.
...
The results show that rSVD achieves its greatest speedup compared to the SVD on matrices of large size, and this speedup is greatest for small target ranks $k$ (Fig.~\ref{fig:run_times_absolute} and Table~\ref{tab:times_all}). This can be explained by the computational complexity of the rSVD, $\mathcal O(kn^2)$, compared to SVD's complexity of $\mathcal O(n^3)$, assuming a matrix of shape $n\times 5n$. As $k$ approaches $n$, the complexity will be approximately $\mathcal O(n^3)$, and the additional computations in the rSVD are no longer beneficial, as was seen for small matrices with target ranks close to 50.
...
Generative AI tools (ChatGPT, OpenAI) have been used for spelling aid and proofreading. All generated content has been reviewed by the authors. The tools did not contribute to ideas, methodology, or results.
...
\begin{equation}
    ...
    \end{bmatrix},
\end{equation}
where $c = \cos\theta$ and $s = \sin\theta$. The method is named after Wallace Givens who introduced it in the 1950s. The matrix can be extended to act on rows $i$ and $j$ of a matrix $A$, and used repeatedly to convert a matrix into a lower triangular form in order to compute a QR factorization, see Section 5.2 of Golub and Van Loan \cite{golub_matrix_1996}.
...
\begin{equation}
    A = A_{(:,J)}X,
\end{equation}
where $J\subset \{1,\dots,n\}$ is an index set of $k$ indices, $A_{(:,J)}$ a matrix made up of the columns of $A$ indexed by $J$, and $X$ a matrix where $X_{(:,J)} = I_k$. See Section 3.2.3 in Halko et al. \cite{halko_finding_2011}.
