...
\begin{equation}
    A = A_{(:,J)}X,
\end{equation}
where $J\subset \{1,\dots,n\}$ is an index set of $k$ indices, $A_{(:,J)}$ a matrix made up of $J$'s columns of $A$, and $X$ a matrix where $X_{(:,J)}=I_k$. See Section 3.2.3 in \cite{halko_finding_2011}.
