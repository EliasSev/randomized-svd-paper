...
\subsection{Effect of Matrix size on Computational Times}

When assessing the effect of the matrix size on the computational time, I compute the median value across all test matrices of a given size and across all three spectral decay sets, since the condition number of the matrix does not affect the computation cost of the SVD and the rSVD (reference). This results in medians computed over $90$ runs for the small case, $45$ runs for the medium case, and $15$ runs for the large case. The results, and the relative computational times for rSVD compared to SVD, can be seen in Figure~\ref{timeAll}, with selected relative values found in Table~\ref{tab:times_all}.
\begin{table}[ht]
\centering
...
For the small case, $n_s=100$, the median computational time for the SVD is 0.025 and 0.021 seconds for wide and tall data, respectively. The computational time for the rSVD if much faster than for SVD for small ranks, $k<10$. For $k=10$ and above, SVD performs better, both on wide data and tall data. The rSVD has the lowest time for small target ranks $k$, and as the rank increases, the computational time increases.

The medium case, where $n_m=500$, the computational time of SVD is higher for all values of $k$ compared to the rSVD. The median computational time for SVD is 0.77 seconds for wide data and 0.57 seconds for tall data. The difference between the two methods reduces as $k$ increases and the computation time for rSVD approaches that of SVD.

For large matrices, with size $n_s = 1000$, the median computational time using the SVD is 4.05 seconds on wide data and 3.15 seconds on tall data. As can be seen in Table~\ref{tab:times_all}, the relative time for rSVD compared to SVD is between 7 times and 32 times as fast, with the greatest speed-up seen for low target ranks.

Comparing the effect of the matrix size, we see that the advantage, in terms of the relative computational time, is greater for larger matrix sizes, both for wide and tall data. In general, a greater advantage is seen on wide data for small matrices, with a greater advantage on tall data for larger ranks, compared to the SVD.
\begin{table}[ht]
\centering
...
\subsection{Effect of Matrix Shape}
Comparing the computational time of rSVD on wide data, $t_{wide}$, to tall data, $t_{tall}$, I look at the ratio $t_{wide} / t_{tall}$ to assess the advantage of performing the method on wide versus tall data. If this value is greater than 1, then $t_{wide} > t_{tall}$, and hence rSVD performs quicker on tall data compared to wide data. The results can be seen in Fig.~\ref{wideVsTall}, and some selected ranks in Table~\ref{tab:times_tall_wide}.

For small values of the target rank $k$, the rSVD performs faster on wide data than for tall data. For $k$ 22, 14, and 8, the performance is faster on wide data than on tall data for the small, medium, and large case, respectively.

For larger values of $k$, rSVD performs better on tall data, and the advantage increases as $k$ increases, as can be seen on the left plot in Fig.~\ref{wideVsTall}. The middle plot, for medium sized matrices, shows that rSVD performs better on tall data for ranks above $k=14$ with approximately 20\%, with not much difference as $k$ increases. The right plot shows that when $k$ is greater than 8, the method is faster on tall data, but the advantage decreases and approaches 1 as $k$ gets closer to 50.

In general, the computational time of rSVD if faster on wide data for small values of $k$, and faster on tall data for larger ranks. In addition, for larger matrices, the advantage of performing rSVD on tall data decreases as $k$ gets larger, while the advantage increases for smaller data.


\subsection{Effect of Spectral Decay on Approximation Error}

The computation of error is done using the $\ell_2$ operator norm, and using the average value across all test matrices of a given size and spectral decay type. The spectral decay of a matrix does not change if it is transposed, and the rSVD method yield approximately the same results on the transpose of the data. The results are therefore done using the wide data, as the results are equivalent on the transposed case.
...
\begin{tabular}{cccccccccc}
    ...
    49 & 1.3 & 1.1 & 1.0 & 2.5 & 1.8 & 1.4 & 2.7 & 2.5 & 2.2 \\ 
    \bottomrule
\end{tabular}
\end{table}
...
\begin{figure*}[tbp]
    ...
    \caption{Average errors for rSVD and best possible error achieved by the SVD over a range of rank-$k$ approximates, for different spectral decays and matrix sizes. Top row: The $\ell_2$ error for both methods. Bottom row: the relative $\ell_2$ error for rSVD compared to SVD.}
    \label{errorsAll}
\end{figure*}
