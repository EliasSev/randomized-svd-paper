...
\end{equation}
where $\sigma_{k+1}$ is the $(k+1)$th singular value of $A$. Equation \eqref{eq:svd_error} says that any rank-$k$ approximation of $A$ can not have an error smaller than $\sigma_{k+1}$.

Let $A\in\mathbb R^{m\times n}$, and pick a target rank $k\ge 2$ and an oversampling parameter $p\ge 2$, where $p+k\le\min\{m,n\}$. Draw a random matrix $\Omega\in\mathbb R^{m\times (k+p)}$, with independent and identically distributed (IID) elements from the standard normal distribution, and use Alg. \ref{alg:rsvd} to obtain a rank-$k$ approximation $X = U\Sigma V^T$. Then,
...
As seen in Fig.~\ref{errorsAll}, the approximation error for rSVD and SVD depends on spectral decay and target rank. We saw that a rapid decay results in more accurate low-rank approximations and lower errors, while slowly decaying spectra require a higher target rank the achieve the same error. For the SVD, this is a direct result of the Eckart–Young–Mirsky theorem. For the rSVD, the error bound \eqref{eq:rsvd_error}, tells us the same: a slowly decaying spectral decay results in a higher expected upper bound on the error.

Fig.~\ref{expectedError} illustrates the severe overestimate of \eqref{eq:rsvd_error}, especially for slow and moderate spectral decay profiles. The bound is on the order of $10^3$ to $10^1$, while the observed error is on the order of $10^1$ to $10^0$. The bounds for rapid decay singular values are tighter, but still overestimates by an order of magnitude. 

For a greater insight into the estimate in \eqref{eq:rsvd_error}, we must investigate $Y = A\Omega$ in Algorithm~\ref{alg:rsvd}. Let $A=U\Sigma V^T$ be the SVD of $A$, and let $\Sigma_1$ consist of the $k+1$ first singular values of $A$, and $\Sigma_2$ the remaining singular values, with $V_1$ and $V_2$ consisting of corresponding right singular vectors, respectively. Then we can write $A$ as
\begin{equation}
    A =
    U
    \begin{bmatrix}
    ...
    \end{bmatrix}.
\end{equation}
$Y$, which represents a random sample of the column space of $A$, takes the form
\begin{equation}
    Y = 
    U
    \begin{bmatrix}
    ...
    \end{bmatrix},
\end{equation}
where $\Omega_1 = V_1^T\Omega$ and $\Omega_2 = V_2^T\Omega_2$. Intuitively, $\Sigma_1\Omega_1$ represents the approximate actions of $A$, while $\Sigma_2\Omega_2$ represents perturbation. Indeed, with a slow decay of singular values, the perturbation $\Sigma_2\Omega_2$ will be more significant, resulting a larger upper bound on the expected error.


\subsection{Interpretation of relative approximation error}
The relative error between rSVD and the SVD approximation (Fig.~\ref{errorsAll}, bottom row) saw the the largest value for small matrices and the smallest values for larges. If we divide \eqref{eq:rsvd_error} by $\sigma_{k+1}$, we get an estimation on the upper bound of the expected error ratio, see Fig.~\ref{gammas}. Surprisingly, we see that this estimate suggests that larger matrices yields greater relative error, which is opposite of the observed results. It is possible that duo to a larger random matrix $\Omega$ in Algorithm~\ref{alg:rsvd} gives greater uncertainty in the estimate, yielding a worse bound. I note that further analysis must is required to understand this behavior.

Another surprising observation is the effect of spectral decay on the relative error. In the results, we saw that a more rapid decay of singular values gave a greater relative error. However, Fig.~\ref{expectedError} shows that opposite trend, where a more rapid decay yields a smaller relative error.
\input{figures/expected_ratio.tex}
