...
When computing the effect of the matrix size on the computational times, the median value is taken over all test matrices of a given size and over the three different spectral decays, as the condition number of the matrix does not affect the computation of the SVD and the rSVD (reference). The values is therefore the median over $90$ runs for the small case, $45$ runs for the medium case, and $15$ runs for the large case. The results, and the relative timings, can be seen in Figure~\ref{timeAll}.

[TODO: insert table with some selected $k$, and the exact median computational time, which I may refer to.]
