...
The singular value decomposition (SVD) is a fundamental technique in data analysis and numerical linear algebra. Its applications are many: visualization of high dimensional data, dimensionality reduction, data compression, regularization techniques, to name a few. The SVD decomposes a matrix $A\in\mathbb R^{m\times n}$ into it's fundamental parts, revealing the main components of the matrix. The computational cost of the SVD scales cubicly (??) for $n\times n$ matrices, making it computationally infeasible in many applications with large matrices. The randomized SVD (rSVD) solves this using randomized algorithms, relying on random matrix theory to compute an rank-$k$ approximate SVD. By taking random samples of the column space of $A$, and approximate orthonormal basis $Q$ is constructed. The matrix $A$ is projected onto it's approximate column space, reducing it's size, but keeping the main information in the matrix. 

% The goal is this paper
