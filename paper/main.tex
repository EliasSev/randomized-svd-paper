...
The relative error between rSVD and the SVD approximation (Fig.~\ref{errorsAll}, bottom row) saw the the largest value for small matrices and the smallest values for larges. As we saw from \eqref{eq:rsvd_perturbation}, a larger $\Omega$ will give a greater perturbation $\Sigma_2\Omega_2$, increasing the upper bound. However, dividing the upper bound the the SVD error $\sigma_{k+1}$ does give a meaningful estimate on this relative error: the the upper bound will increase while $\sigma_{k+1}$ stays fixed. 

In the case of relative error in relation to the spectral decay profile, we saw that a rapid decay gave a higher relative error. However, even if the relative error is greater, the absolute error is the smallest for such spectra, which illustrates how rSVD and SVD both a most effective when the dominant amount of actions of $A$ are captured in the first few singular vectors.
