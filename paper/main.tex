...
For larger values of $k$, rSVD performs better on tall data, and the advantage increases as $k$ increases, as can be seen on the left plot in Fig.~\ref{wideVsTall}. The middle plot, for medium sized matrices, shows that rSVD performs better on tall data above $k=14$ with approximately 20\%, with not much difference as $k$ increases. The right plot shows that when $k$ is greater than 8, the method is faster on tall data, but the advantage decreases and approaches 1 as $k$ gets closer to 50.
