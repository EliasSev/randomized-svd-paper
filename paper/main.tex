...
\end{equation}
where the parameter $\alpha$ controls the rate of decay. The parameters used in this study  are $\alpha_1=0.01$, $\alpha_2 = 0.2$ and $\alpha_3 = 0.75$, which corresponds to a slow, medium and a rapid spectral decay, respectively (see Fig.~\ref{spectralDecay}).

\begin{figure}[htbp]
    \centerline{\includegraphics[width=0.57\textwidth]{singular_value_decay.png}}
    \caption{The slow, medium, and rapid spectral decay used for the test matrix generation.}
    \label{spectralDecay}
\end{figure}
